 ...
 * Canonical live schema for quotation date: tgl_penawaran.
 * No schema probing, no fallback that removes columns, and no harga_modal in quotations.
 */
(function () {
  'use strict';
  ...
  async function saveClient(client) {
    const result = await db().from(DB.clients).insert({
  ...
  async function saveHeader(database, form, client, total) {
    ...
    /*
     * These are intentional core columns. Do not remove them on error.
     * tgl_penawaran is the live database column reported by Supabase.
     * nama_event is also written because older/live penawaran schemas require it.
     */
    const payload = {
    ...
    const result = await database.from(DB.quotations).insert(payload).select(
      'id,nomor_penawaran,nama_client,perusahaan,telepon,whatsapp,email,nama_event,event_name,tanggal_mulai,tanggal_selesai,tgl_penawaran,total,status'
    ).single();
  ...
  async function saveItem(database, quotationId, item) {
    ...
    }).select('id,penawaran_id,kode,item,harga_jual,tipe_perhitungan,qty,lebar,tinggi,panjang,tanggal_mulai,tanggal_selesai,durasi,subtotal').single();
  ...
  async function saveSchedule(database, itemId, item) {
    const result = await database.from(DB.schedules).insert({
      ...
      durasi: item.durasi,
      subtotal: item.subtotal
    }).select('id,penawaran_item_id,qty,tanggal_mulai,tanggal_selesai,durasi,subtotal').single();
  ...
  async function saveQuoteFixed() {
    ...
    try {
      ...
      // Client is intentionally saved first so even a DRAFT is available for follow-up.
