 ...
 * Keeps Invoice business logic intact while matching the customer-facing
 * item/table presentation used by the quotation.
 */
(function(){
  'use strict';
  ...
  function period(start,end,short){
    const a=dateParts(start),b=dateParts(end)||a;if(!a)return S(start)||'-';
  ...
  const findMaster=code=>(window.masters||[]).find(m=>S(m.kode)===S(code))||null;

  function packageHtml(master){
    const rows=parsePackage(master?.isi_paket);if(!rows.length)return '';
  ...
  function installStyles(){
    ...
    const st=document.createElement('style');st.id='pmInvoiceQuoteSyncStyles';st.textContent=`
      ...
      #pmInvoicePreview .pm-inv-sync-table tbody tr:nth-child(even):not(.pm-inv-sync-total) td{background:#f8fafc!important}
      ...
      #pmInvoicePreview .pm-invoice-package{margin-top:5px!important;padding:5px 8px 2px 12px!important;color:#27334a!important;border-left:3px solid #7ea1ff!important}
      #pmInvoicePreview .pm-invoice-package-title{font-size:7px!important;font-weight:800!important;letter-spacing:.08em!important;margin-bottom:2px!important;color:#54627c!important}
      #pmInvoicePreview .pm-invoice-package table{width:100%!important;border-collapse:collapse!important;font-size:7px!important}
      #pmInvoicePreview .pm-invoice-package th,#pmInvoicePreview .pm-invoice-package td{padding:1.5px 4px!important;border:0!important;text-align:left!important;vertical-align:top!important}
      #pmInvoicePreview .pm-invoice-package th{color:#66748e!important;font-weight:800!important;text-transform:uppercase!important;font-size:6.4px!important}
      #pmInvoicePreview .pm-invoice-package th:last-child,#pmInvoicePreview .pm-invoice-package td:last-child{width:48px!important;text-align:right!important;font-weight:600!important}
      #pmInvoicePreview .pm-invoice-package td{color:#334155!important}
      #pmInvoicePreview .pm-inv-sync-item>td:nth-child(2){vertical-align:top!important}
  ...
  async function transform(){
    ...
    try{
      ...
      const quote=await d.from('penawaran').select('id,nama_event,event_name,tanggal_mulai,tanggal_selesai,total,grand_total').eq('nomor_penawaran',quoteNumber).maybeSingle();
      ...
      const items=ir.data||[];
      let extras=[];try{const er=await d.from('penawaran_invoice_items').select('*').eq('penawaran_id',q.id).order('id',{ascending:true});if(!er.error)extras=er.data||[]}catch(_){ }
      ...
      const rowHtml=items.map((item,i)=>{
        ...
        const pkg=master&&S(master.isi_paket)?packageHtml(master):'';
        return `<tr class="pm-inv-sync-item"><td class="center">${i+1}</td><td><strong>${E(item.item||item.nama_item||'-')}</strong><div class="pm-inv-sync-code">${E(code)}</div>${pkg}</td><td class="center">${E(itemQty(item))}</td><td class="center pm-inv-sync-schedule">${E(sched)}</td><td class="right pm-inv-sync-nowrap">${M(item.harga_jual??item.harga)}</td><td class="right pm-inv-sync-nowrap">${M(item.subtotal)}</td></tr>`;
      }).join('');
      const extraHtml=extras.map(item=>`<tr class="pm-inv-sync-item"><td class="center">+</td><td><strong>${E(item.nama_item||item.item||'Item Tambahan')}</strong><div class="pm-inv-sync-code">${E(item.kode||'ADD-INV')}</div></td><td class="center">${E(item.tipe_perhitungan==='overtime'?`${N(item.qty)} jam`:`${N(item.qty)||1} ${S(item.satuan||'unit')}`)}</td><td class="center pm-inv-sync-schedule">${E(period(item.tanggal_mulai,item.tanggal_selesai,true))}</td><td class="right pm-inv-sync-nowrap">${M(item.harga)}</td><td class="right pm-inv-sync-nowrap">${M(item.subtotal)}</td></tr>`).join('');
      const total=N(q.grand_total??q.total),table=document.createElement('table');
      table.className='pm-inv-sync-table';
