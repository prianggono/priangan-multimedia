...
(function(){
  ...
  function baseNumber(eventName, seed){
    const year=new Date().getFullYear();
    const digits=String(seed??Date.now()).replace(/\D/g,'').slice(-6).padStart(6,'0');
    return `PM-${slug(eventName)}-${year}-${digits}`;
  }

  function splitRevision(no){
    const m=S(no).match(/^(.*?)(?:_(\d+))?$/);
    return {base:m?.[1]||S(no), rev:m?.[2]?Number(m[2]):0};
  }

  function nextRevision(no){
    const x=splitRevision(no);
    return `${x.base}_${x.rev+1}`;
  }

  async function quoteById(id){
    const d=dbx();if(!d||!id)return null;
    try{
      const r=await d.from('penawaran').select('id,nomor_penawaran,nama_event,nama_client,perusahaan').eq('id',id).maybeSingle();
  ...
  async function normalizeSavedQuote(id, isEdit, oldRow, eventName){
    ...
    if(isEdit){
      const oldNo=S(oldRow?.nomor_penawaran);
      number=oldNo ? nextRevision(oldNo) : baseNumber(eventName||row.nama_event,id);
    }else if(!/^PM-/i.test(number) || /^PM-\d{6}$/i.test(number)){
      number=baseNumber(eventName||row.nama_event,id);
    }else{
      // Replace the temporary legacy timestamp number created by the old save layer.
      const p=number.split('-');
      if(p.length===2 && /^\d+$/.test(p[1])) number=baseNumber(eventName||row.nama_event,id);
    }
  ...
  function installSave(){
    ...
    const wrapped=async function(){
      ...
      let oldRow=null;
      if(editing) oldRow=await quoteById(editing);
      ...
      try{
        if(editing){
          ...
          if(no) toast(`Penawaran ${no} berhasil diperbarui.`);
        }else{
          const d=dbx();
          if(d){
            ...
            if(!r.error&&r.data) await normalizeSavedQuote(r.data.id,false,null,eventName||r.data.nama_event);
          }
        }
    ...
    const wrapped=async function(id){
      ...
      try{
        ...
        if(!r.error&&r.data&&r.data.nomor_penawaran){
          ...
          // Existing invoice number is preserved. New invoice follows quotation number.
  ...
  function installQuotationPrint(){
    ...
    const wrapped=async function(){
      const editing=N(window.__pmEditingQuotationId||window.__PM_EDIT_QUOTATION_ID);
      let number=S(window.__PM_LAST_QUOTATION_NUMBER);
      ...
      document.title=`Penawaran - ${number}`;
      const result=await original.apply(this,arguments);
      const root=document.getElementById('pmPrintPreview');
      if(root){
        const tag=root.querySelector('.pm-doc-tag strong');
        if(tag)tag.textContent=number;
        const span=root.querySelector('.pm-print-toolbar span');
        if(span)span.textContent=`A4 Portrait • ${number}`;
      }
      window.__PM_PRINT_OLD_TITLE=oldTitle;
      return result;
    };
    wrapped.__pmDocumentNumbering=true;
  ...
  function installPrintTitle(){
    if(typeof window.executePrintPreview!=='function')return false;
    if(window.executePrintPreview.__pmDocumentNumbering)return true;
    const original=window.executePrintPreview;
    const wrapped=async function(){
      const oldTitle=document.title;
      const no=S(window.__PM_PRINT_DOCUMENT_NUMBER);
  ...
  function installInvoicePreview(){
    ...
    const wrapped=async function(){
      ...
      const oldTitle=document.title;
      if(no)document.title=`Invoice - ${no}`;
      const result=await original.apply(this,arguments);
      const root=document.getElementById('pmInvoicePreview');
      if(root&&no){
        const els=root.querySelectorAll('.pm-inv-number,.pm-doc-tag strong,[data-pm-invoice-number]');
        els.forEach(el=>el.textContent=no);
      }
  ...
  function boot(){
    installSave();installInvoiceEdit();installQuotationPrint();installPrintTitle();installInvoicePreview();
  }
  boot();
