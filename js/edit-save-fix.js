 ...
 * Items are synchronized by product code so adding/removing items works.
 * IMPORTANT: only columns known to exist in the live penawaran insert contract are written.
 */
(function(){
'use strict';
...
async function saveEdit(){
 const quoteId=S(window.__pmEditingQuotationId||window.__PM_EDIT_QUOTATION_ID);if(!quoteId)return false;const d=DB();if(!d){toast('Supabase belum terhubung.');return true}
 ...
 try{
  ...
  const base=current.reduce((a,x)=>a+N(x.subtotal),0);

  /* CRITICAL FIX: the live penawaran table does NOT have the old discount/discount_percent columns.
     Writing any of them makes Supabase return HTTP 400 before penawaran_items can be synchronized.
     Only use columns already used by the working CREATE quotation flow. */
  const updatePayload={
   nama_client:f.nama_client,perusahaan:f.perusahaan,telepon:f.telepon,whatsapp:f.whatsapp,email:f.email,
   nama_event:f.nama_event,event_name:f.nama_event,tanggal_mulai:f.tanggal_mulai,tanggal_selesai:f.tanggal_selesai,
   subtotal:base,total:base,grand_total:base
  };
  let r=await d.from('penawaran').update(updatePayload).eq('id',quoteId).select('id').single();
  ...
  window.__pmEditingQuotationId=null;window.__PM_EDIT_QUOTATION_ID=null;toast('Penawaran berhasil diperbarui. Item baru tersimpan.');if(typeof window.go==='function')window.go('history');return true;
...
const original=window.saveQuote;window.saveQuote=async function(){if(window.__pmEditingQuotationId||window.__PM_EDIT_QUOTATION_ID)return saveEdit();return typeof original==='function'?original():toast('Fungsi simpan penawaran tidak tersedia.')};
document.addEventListener('click',e=>{const b=e.target.closest?.('button[onclick="saveQuote()"]');if(!b||(!window.__pmEditingQuotationId&&!window.__PM_EDIT_QUOTATION_ID))return;e.preventDefault();e.stopImmediatePropagation();saveEdit()},true);
