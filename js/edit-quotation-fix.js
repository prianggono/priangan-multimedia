...
function savedDiscountPercent(row,discount){
  ...
  if(total>0&&discount>0){const inferredBase=total+discount;return Math.max(0,Math.min(100,Math.trunc((discount/inferredBase)*100)))}
  return 0;
}
...
async function persistSavedDiscount(d,id,state){
  if(!d||!id)return;
  const payload={subtotal:state.base,diskon:state.discount,diskon_persen:state.pct,diskon_nominal:state.discount,total:state.total,grand_total:state.total};
...
async function restoreDiscount(row){
  ...
  window.__PM_DISC_MODE='pct';window.__pmDiscountPct=savedPct;window.__pmDiscountValue=savedDiscount;window.__pmDiscountBase=Math.max(0,N(row?.subtotal));
  if(p){p.value=String(savedPct);p.dispatchEvent(new Event('input',{bubbles:true}))}
  if(r){r.value=savedDiscount;r.dispatchEvent(new Event('change',{bubbles:true}))}
  window.__pmDiscountPct=savedPct;window.__pmDiscountValue=savedDiscount;
}
async function editQuotationFixed(id){
 ...
 try{
  ...
  window.__pmEditingQuotationId=id;window.__pmEditingQuotationNumber=S(row.nomor_penawaran||row.nomor||id);window.go('quotation');await wait(180);
  ...
  await wait(40);await restoreDiscount(row);await wait(80);await restoreDiscount(row);
...
function installSavePersistence(){
  if(typeof window.saveQuote!=='function'||window.saveQuote.__pmDiscountPersistenceV2)return false;
  const original=window.saveQuote;
  const wrapped=async function(){
    const d=DB(),editId=N(window.__pmEditingQuotationId||window.__PM_EDIT_QUOTATION_ID),beforeState=readDiscountState();
    const beforeIds=new Set();
    if(d){try{const q=await d.from('penawaran').select('id').order('id',{ascending:false}).limit(500);(q.data||[]).forEach(x=>beforeIds.add(String(x.id)))}catch(_){}
    }
    const result=await original.apply(this,arguments);
    if(!d)return result;
    try{
      let targetId=editId;
      if(!targetId){const q=await d.from('penawaran').select('id').order('id',{ascending:false}).limit(20);const fresh=(q.data||[]).find(x=>!beforeIds.has(String(x.id)))||q.data?.[0];targetId=N(fresh?.id)}
      if(targetId)await persistSavedDiscount(d,targetId,beforeState);
    }catch(e){console.error('[PM] discount persistence:',e);toast('Penawaran tersimpan, tetapi sinkronisasi diskon gagal: '+(e.message||e))}
    return result;
  };
  wrapped.__pmDiscountPersistenceV2=true;window.saveQuote=wrapped;return true;
}
window.editQuotation=editQuotationFixed;window.__PRIANGAN_EDIT_QUOTATION_FIXED=true;installSavePersistence();[100,300,700,1500,3000,5000].forEach(ms=>setTimeout(installSavePersistence,ms));
})();
