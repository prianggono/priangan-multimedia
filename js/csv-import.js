...
function csvNormalizeHeader(value) {
  return String(value ?? '').replace(/^\uFEFF/, '').trim().toLowerCase().replace(/\s+/g, '_').replace(/[()\-\/]+/g, '_').replace(/_+/g, '_');
}
...
function csvValue(obj, aliases) {
  for (const key of aliases) {
    ...
    if (Object.prototype.hasOwnProperty.call(obj, normalized)) return String(obj[normalized] ?? '').trim();
  }
  return '';
}
...
async function handleCSVImport(e) {
  ...
  // Prevent duplicate handling when both inline onchange and delegated listener fire.
  ...
  try {
    ...
    if (!text.trim()) { msg('File CSV kosong.'); return; }

    const delimiter = csvDetectDelimiter(text);
    const rows = csvParse(text, delimiter);
    if (rows.length < 2) { msg('CSV harus memiliki header dan minimal 1 data.'); return; }
    ...
    for (let i = 1; i < rows.length; i++) {
      const obj = {};
      headers.forEach((h, idx) => obj[h] = String(rows[i][idx] ?? '').trim());

      const kode = csvValue(obj, ['kode', 'code']);
      const item = csvValue(obj, ['item', 'nama_item', 'nama']);
      if (!kode && !item) continue;
      if (!kode || !item) { errors.push(`Baris ${i + 1}: Kode/Item kosong`); continue; }
    ...
    for (let i = 0; i < payload.length; i += batchSize) {
      ...
      if (result.error) {
        ...
        msg('Gagal import: ' + result.error.message);
        return;
      }
    }
    ...
    await load();
    render();
    msg(`Berhasil import ${payload.length} item${errors.length ? `; ${errors.length} baris dilewati` : ''}.`);
    console.log('CSV import sukses:', { delimiter, payload, errors });
  } catch (err) {
...
// Fallback event handler. Ini memastikan import tetap berjalan walaupun
// inline onchange pada form dinamis tidak terpanggil oleh browser.
