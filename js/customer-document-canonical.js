 ...
 * All item subtotals are recalculated from saved price/qty/dates so missing legacy subtotal
 * values can never render as Rp 0.
 */
(function(){
  'use strict';
  ...
  const calcBase=item=>{
    ...
    const start=item.tanggal_mulai,end=item.tanggal_selesai,d=days(start,end);
    ...
    if(isLED(item)){
      ...
      const level=item.level_enabled?N(item.lebar)*Math.max(0,N(item.level_harga))*qty:0;
      return Math.max(0,led+level);
    }
    if(t==='level')return Math.max(0,N(item.lebar)*price*qty);
    if(t==='rigging')return Math.max(0,(N(item.panjang)*2+N(item.tinggi)*2)*price*qty*d);
    if(t==='overtime')return Math.max(0,N(item.qty)*price);
    return Math.max(0,qty*price*d);
  };
  const itemNet=item=>{const base=calcBase(item),pct=Math.max(0,Math.min(100,N(item.diskon_persen))),rp=pct>0?Math.min(base,Math.round(base*pct/100)):Math.min(base,Math.max(0,N(item.diskon_nominal)));return Math.max(0,base-rp);};
  ...
  const packageHtml=item=>{const m=findMaster(item?.kode),rows=packageRows(m?.isi_paket);if(!rows.length)return '';return `<div class="pm-cust-package"><div class="pm-cust-package-title">ISI PAKET</div><table><thead><tr><th>Komponen</th><th>Qty</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${E(r.name)}</td><td>${E(r.qty)}</td></tr>`).join('')}</tbody></table></div>`;};
  ...
  style();refresh();
  window.__PM_CUSTOMER_DOCUMENT_API={refresh,nameFor,itemBase:calcBase,itemNet};
})();