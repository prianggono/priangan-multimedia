...
(function(){
  ...
  const itemBaseSubtotal=item=>{
    const price=N(item?.harga_jual??item?.harga),qty=Math.max(1,N(item?.qty??item?.jumlah)||1),duration=days(item?.mulai??item?.tanggal_mulai,item?.selesai??item?.tanggal_selesai),width=N(item?.lebar),height=N(item?.tinggi),length=N(item?.panjang),t=S(item?.tipe_perhitungan||item?.tipe).toLowerCase();
    if(isLED(item))return width*height*price*qty*duration+levelSubtotal(item);
    if(t==='rigging')return ((length*2)+(height*2))*price*duration;
    if(t==='level')return 0;
    return qty*price*duration;
  };

  const itemDiscount=item=>{
    const led=isLED(item);
    const savedSubtotal=N(item?.subtotal);
    const base=led?itemBaseSubtotal(item):(savedSubtotal>0?savedSubtotal:itemBaseSubtotal(item));
  ...
  const qtyText=item=>{
    ...
    if(isLED(item)){
      ...
      return `${N(item.lebar)} × ${N(item.tinggi)} m²${set>1?` • ${set} set`:''}`;
    }
  ...
  const packageHtml=item=>{
    const m=masters().find(x=>S(x.kode)===S(item?.kode)),rows=packageRows(m?.isi_paket);if(!rows.length)return '';
    return `<div class="pm-cust-package"><div class="pm-cust-package-title">ISI PAKET</div><table><thead><tr><th>Komponen</th><th>Qty</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${E(r.name)}</td><td>${E(r.qty)}</td></tr>`).join('')}</tbody></table></div>`;
  };
  const priceHtml=item=>{
    const p=M(item?.harga_jual??item?.harga);
    return item?.level_enabled&&N(item.level_harga)>0?`${E(p)}<div class="pm-cust-level-price">Level: ${E(M(item.level_harga))}/m</div>`:E(p);
  };

  function style(){
    if(document.getElementById('pmCustomerDocumentStyles'))return;
    const st=document.createElement('style');st.id='pmCustomerDocumentStyles';
    st.textContent='.pm-cust-level-price{font-size:6.5pt!important;color:#475569!important;margin-top:2px!important;line-height:1.1!important}.pm-cust-package{margin-top:5px;padding:5px 8px 2px 12px;border-left:3px solid #7ea1ff;color:#334155}.pm-cust-package-title{font-size:7px;font-weight:800;letter-spacing:.08em;color:#64748b;margin-bottom:2px}.pm-cust-package table{width:100%;border-collapse:collapse;font-size:7px}.pm-cust-package th,.pm-cust-package td{padding:1.5px 4px;border:0;text-align:left;vertical-align:top}.pm-cust-package th{font-size:6.4px;color:#64748b;text-transform:uppercase}.pm-cust-package th:last-child,.pm-cust-package td:last-child{width:48px;text-align:right;font-weight:600}.pm-cust-total td{background:#edf4ff!important;font-weight:800!important;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}';
    document.head.appendChild(st);
  }

  function normalizeSavedItem(r){
    return {
      ...r,
      harga:N(r.harga??r.harga_jual),harga_jual:N(r.harga_jual??r.harga),qty:Math.max(1,N(r.qty??r.jumlah)||1),jumlah:Math.max(1,N(r.jumlah??r.qty)||1),
      lebar:N(r.lebar),tinggi:N(r.tinggi),panjang:N(r.panjang),
      mulai:S(r.tanggal_mulai||''),selesai:S(r.tanggal_selesai||''),
      tipe:S(r.tipe_perhitungan||r.tipe||''),tipe_perhitungan:S(r.tipe_perhitungan||r.tipe||''),
      level_enabled:!!r.level_enabled,level_master_harga_id:r.level_master_harga_id??null,level_tinggi:N(r.level_tinggi),level_harga:N(r.level_harga),level_subtotal:N(r.level_subtotal)
    };
  }

  async function savedQuotationItems(root){
    const d=db();if(!d)return null;
    let qid=Number(window.__pmEditingQuotationId||window.__PM_EDIT_QUOTATION_ID||0);
    try{
      if(!qid){
        const txt=S(root?.textContent),match=txt.match(/PM-\d{4}-\d+/i),no=match?.[0];
        if(no){const qr=await d.from('penawaran').select('id').eq('nomor_penawaran',no).maybeSingle();if(!qr.error&&qr.data)qid=Number(qr.data.id);}
      }
      if(!qid)return null;
      const ir=await d.from('penawaran_items').select('*').eq('penawaran_id',qid).order('id',{ascending:true});
      if(ir.error||!Array.isArray(ir.data)||!ir.data.length)return null;
      return ir.data.map(normalizeSavedItem);
  ...
  function hasPackageMarkup(cell){return !!cell?.querySelector('.pm-cust-package,.pm-package-print');}
  ...
  function patchQuotationRows(root,list){
    ...
    list.forEach((item,i)=>{
      ...
      if(!hasPackageMarkup(cells[1])&&packageHtml(item))cells[1].insertAdjacentHTML('beforeend',packageHtml(item));
    });
  ...
  async function invoiceRowsFromDb(area){
    const d=db();if(!d||!area)return null;
    const txt=S(area.textContent),no=(txt.match(/PM-\d{4}-\d+/i)||[])[0];if(!no)return null;
    try{
      const qr=await d.from('penawaran').select('*').eq('nomor_penawaran',no).maybeSingle();if(qr.error||!qr.data)return null;
      const ir=await d.from('penawaran_items').select('*').eq('penawaran_id',qr.data.id).order('id',{ascending:true});if(ir.error)return null;
      let extras=[];try{const er=await d.from('penawaran_invoice_items').select('*').eq('penawaran_id',qr.data.id).order('id',{ascending:true});if(!er.error)extras=er.data||[]}catch(_){}
      return{quote:qr.data,items:(ir.data||[]).map(normalizeSavedItem),extras};
    }catch(_){return null;}
  }

  async function invoicePatch(){
    const area=document.querySelector('#pmInvoicePreview');if(!area)return;
    const data=await invoiceRowsFromDb(area);if(!data)return;
    const table=area.querySelector('.pm-inv-table');if(!table)return;const body=table.tBodies[0];if(!body)return;
    const baseRows=[...body.querySelectorAll('tr')].filter(r=>!r.classList.contains('pm-inv-total')&&!r.classList.contains('pm-inv-extra-total')&&!r.classList.contains('pm-cust-extra'));
    data.items.forEach((item,i)=>{
      const row=baseRows[i];if(!row)return;const cells=row.cells;if(cells.length<5)return;
      const name=cells[1].querySelector('strong');if(name)name.textContent=displayName(item);
      const code=cells[1].querySelector('.code');if(code)code.textContent=S(item.kode);
      if(!hasPackageMarkup(cells[1])&&packageHtml(item))cells[1].insertAdjacentHTML('beforeend',packageHtml(item));
      cells[2].textContent=qtyText(item);cells[3].innerHTML=priceHtml(item);cells[4].textContent=M(itemDiscount(item));
    });
    const extras=(data.extras||[]).reduce((s,x)=>s+N(x.subtotal),0),total=data.items.reduce((s,x)=>s+itemDiscount(x),0)+extras;
    const totalRow=body.querySelector('.pm-inv-total');if(totalRow?.cells?.length)totalRow.cells[totalRow.cells.length-1].textContent=M(total);
  ...
  document.addEventListener('click',e=>{
    ...
    if(/Preview Surat|Surat Penawaran|Cetak.*PDF|Preview \/ Cetak A4/i.test(t))scheduleQuotation();
    if(/Preview Invoice|Cetak.*Invoice/i.test(t))scheduleInvoice();
  },true);
