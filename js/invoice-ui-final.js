...
<div class="pmx-field pmx-hide" id="pmxLengthWrap"><label>Panjang Rigging (m)</label><input id="pmxLength" type="number" min="0" step="0.01" value="0"></div>
    ...
    const q=id=>el.querySelector(id),master=q('#pmxMaster'),name=q('#pmxName'),type=q('#pmxType'),price=q('#pmxPrice'),st=q('#pmxStart'),en=q('#pmxEnd'),qty=q('#pmxQty'),w=q('#pmxWidth'),h=q('#pmxHeight'),len=q('#pmxLength');
    ...
    q('.pmx-save').onclick=()=>{if(typeof window.invoiceSaveAddItem!=='function'){alert('Fungsi penyimpanan item invoice belum siap.');return;}window.invoiceSaveAddItem();};
    show();refresh();
  }
