...
(function () {
  ...
  async function previewInvoice() {
    ...
    try {
      ...
      root.innerHTML = `<div class="pm-inv-toolbar"><div><strong>Preview Invoice</strong><span>A4 Portrait • ${E(invoiceNo)}</span></div><div class="pm-inv-actions"><button class="pm-inv-zoom" type="button" id="pmInvZoomOut">−</button><span class="pm-inv-zoom-value">Fit</span><button class="pm-inv-zoom" type="button" id="pmInvZoomIn">+</button><button class="btn secondary" type="button" id="pmInvFit">Fit</button><button class="btn secondary" type="button" id="pmInvClose">Tutup</button><button class="btn green" type="button" id="pmInvPrint">Cetak / Simpan PDF</button></div></div><div class="pm-inv-scroll"><div class="pm-inv-stage"><main class="pm-inv-a4 pm-order-density-${density}"><header class="pm-letterhead pm-inv-letterhead"><div class="pm-logo-wrap"></div><div class="pm-brand"></div><div class="pm-doc-tag"><span>INVOICE</span><strong>${E(invoiceNo)}</strong></div></header><section class="pm-info-card pm-inv-info"><div class="pm-info-section pm-inv-box"><div class="pm-section-label">DITUNJUKAN KEPADA</div><div class="pm-client-name">${E(quote.nama_client || '-')}</div><div>${E(quote.perusahaan || '')}</div><div>${E(quote.telepon_wa || quote.telepon || quote.whatsapp || '')}</div><div>${E(quote.email || '')}</div></div><div class="pm-info-section pm-event-section pm-inv-box"><div class="pm-section-label">EVENT / PROJECT</div><div class="pm-event-name">${E(quote.nama_event || quote.event_name || '-')}</div><div class="pm-period-label">PERIODE</div><div>${E(periodText(quote.tanggal_mulai, quote.tanggal_selesai))}</div></div></section></section><table class="pm-inv-table"><thead><tr><th>No.</th><th>Produk / Jasa</th><th>Qty / Dimensi</th><th>Jadwal</th><th>Harga</th><th>Subtotal</th></tr></thead><tbody>${rows}${extraRows}<tr class="pm-inv-extra-total"><td colspan="5" class="right">Tambahan Invoice</td><td class="right">${M(extrasTotal(d.ex))}</td></tr><tr class="pm-inv-total"><td colspan="5" class="right"><strong>TOTAL</strong></td><td class="right">${M(total)}</td></tr></tbody></table><section class="pm-inv-pay"><div class="pm-inv-paybox"><div class="pm-inv-label">PEMBAYARAN</div><div class="pm-inv-payrow"><span>Total Invoice</span><b>${M(total)}</b></div><div class="pm-inv-payrow"><span>Downpayment</span><b>${M(paid(d.p))}</b></div><div class="pm-inv-payrow"><span>Sisa</span><b class="pm-inv-balance">${M(Math.max(0, total - paid(d.p)))}</b></div></div><div class="pm-inv-paybox"><div class="pm-inv-label">REKENING</div><div class="pm-inv-bank-details"><span>Bank</span><b>BCA • 1394575278</b><span>a.n.</span><b>Ganjar Rahmadhita</b></div></div></section><div class="pm-inv-bottom-row">${template.nama_penandatangan ? `<div class="pm-inv-sign"><div>HORMAT KAMI,</div>${template.ttd_url ? `<img src="${E(template.ttd_url)}" alt="TTD">` : ''}<div class="pm-inv-line"></div><strong>${E(template.nama_penandatangan)}</strong><div>${E(template.jabatan_penandatangan || '')}</div></div>` : ''}</div><div class="pm-inv-footer">Terima kasih atas kepercayaan dan kesempatan yang diberikan kepada PRI_ERP.<strong>PRI_ERP</strong></div></main></div></div>`;
      document.body.appendChild(root);
      ...
      const invScroll=root.querySelector('.pm-inv-scroll'), invStage=root.querySelector('.pm-inv-stage'), invA4=root.querySelector('.pm-inv-a4');
      const applyInvZoom=(value,fit=false)=>{
        if(!invScroll||!invStage||!invA4)return;
        const w=invA4.offsetWidth||794,h=invA4.offsetHeight||1123,v=invScroll.getBoundingClientRect();
        const zFit=Math.max(.25,Math.min(1,(v.width-2)/w,(v.height-2)/h));
        const z=fit?zFit:Math.max(zFit,Math.min(1.8,Number(value)||zFit));
        window.__PM_INVOICE_ZOOM=z; invA4.style.transform=`scale(${z})`; invA4.style.transformOrigin='top left';
        invStage.style.width=Math.round(w*z)+'px'; invStage.style.height=Math.round(h*z)+'px';
        invStage.style.margin='0 auto'; root.querySelector('.pm-inv-zoom-value').textContent=Math.round(z*100)+'%';
      };
      const fitInv=()=>applyInvZoom(0,true);
      root.querySelector('#pmInvZoomOut').onclick=()=>applyInvZoom((window.__PM_INVOICE_ZOOM||.5)-.1);
      root.querySelector('#pmInvZoomIn').onclick=()=>applyInvZoom((window.__PM_INVOICE_ZOOM||.5)+.1);
      root.querySelector('#pmInvFit').onclick=fitInv;
      requestAnimationFrame(fitInv);
      window.addEventListener('resize',fitInv,{passive:true});
