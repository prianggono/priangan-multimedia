...
(function () {
  ...
  const levelAmount = (i) => i?.level_enabled
    ? N(i.lebar) * N(i.level_harga) * Math.max(1, N(i.qty ?? i.jumlah) || 1)
    : 0;
  ...
  function itemName(i) {
    ...
    return i?.level_enabled && cm(i.level_tinggi)
      ? `${base} + Level ${cm(i.level_tinggi)} cm`
      : base;
  }
  ...
  const paid = (rows) => rows.reduce((s, r) => s + N(r.nominal), 0);
  const extrasTotal = (rows) => rows.reduce((s, r) => s + N(r.subtotal), 0);
  ...
  async function invoicePage() {
    ...
    try {
      ...
      const paidMap = new Map(), extraMap = new Map();
      ...
      c.innerHTML = `<div class="head"><div><h1>Invoice</h1><p>Sumber data: Supabase.</p></div><button class="btn secondary" id="pmInvRefresh" type="button">↻ Refresh</button></div><div class="card"><div class="scroll"><table class="table"><thead><tr><th>No. Invoice</th><th>Penawaran</th><th>Client</th><th>Event</th><th>Total</th><th>Dibayar</th><th>Sisa</th><th>Status</th><th>Aksi</th></tr></thead><tbody>${(q.data || []).map((r) => {
        const ex = extraMap.get(r.id) || [], pmt = paidMap.get(r.id) || 0, total = quoteTotal(r) + extrasTotal(ex);
        const status = r.status_invoice === 'DIBATALKAN' ? 'DIBATALKAN' : r.nomor_invoice && pmt >= total && total > 0 ? 'LUNAS' : r.nomor_invoice ? (r.status_invoice || 'DITERBITKAN') : 'BELUM DIBUAT';
        return `<tr><td><strong>${E(r.nomor_invoice || '-')}</strong></td><td>${E(r.nomor_penawaran || '-')}</td><td>${E(r.nama_client || '-')}<div class="muted">${E(r.perusahaan || '')}</div></td><td>${E(r.nama_event || r.event_name || '-')}</td><td>${M(total)}</td><td>${M(pmt)}</td><td>${M(Math.max(0, total - pmt))}</td><td>${E(status)}</td><td><div class="actions"><button class="btn sm" type="button" data-edit="${r.id}">${r.nomor_invoice ? 'Edit / Lihat' : 'Buat Invoice'}</button>${total > pmt ? `<button class="btn sm secondary" type="button" data-pay="${r.id}">${pmt ? 'Pelunasan' : 'Input DP'}</button>` : ''}${r.nomor_invoice ? `<button class="btn sm green" type="button" data-preview="${r.id}">Preview Invoice</button>` : ''}</div></td></tr>`;
  ...
  async function openForm(id) {
    ...
    try {
      ...
      const rows = d.items.map((i, idx) => itemRow(i, idx + 1)).join('') || '<tr><td colspan="6">Tidak ada item.</td></tr>';
      const qTotal = quoteTotal(d.q), total = qTotal + extrasTotal(d.ex);
  ...
  function renderExtras() {
    ...
    c.querySelectorAll('[data-extra-index]').forEach((card) => {
      ...
      card.querySelectorAll('[data-f]').forEach((el) => {
        ...
        if (el.dataset.f === 'tipe_perhitungan') el.value = x.tipe_perhitungan || 'qty';
        el.oninput = () => {
  ...
  function extraSubtotal(x) {
    ...
    const qty = Math.max(1, N(x.qty) || 1), w = N(x.lebar), h = N(x.tinggi), l = N(x.panjang), p = N(x.harga);
  ...
  function updateSummary() {
    ...
    const values = [['#invExtraTotal', extrasTotal(current.ex)], ['#invTotal', total], ['#invBalance', Math.max(0, total - current.paid)]];
    values.forEach(([sel, value]) => { const el = document.querySelector(sel); if (el) el.textContent = M(value); });
  }
  ...
  function invoiceRemoveItem(i) {
    if (!current) return;
    current.ex.splice(i, 1); renderExtras(); updateSummary();
  }
  ...
  async function openPaymentModal(id) {
    ...
    const already = paid(d.p), balance = Math.max(0, total - already);
    ...
    const save = async () => {
      const nominal = N(document.querySelector('#pmPaymentAmount')?.value), method = S(document.querySelector('#pmPaymentMethod')?.value || 'Transfer');
  ...
  async function previewInvoice() {
    ...
    root.querySelector('#pmInvPrint').onclick = () => {
      ...
      document.title = quote.nomor_invoice || 'Invoice';
      window.print();
  ...
  if (nav && !nav.dataset.pmInvoiceCore) {
    nav.dataset.pmInvoiceCore = '1';
