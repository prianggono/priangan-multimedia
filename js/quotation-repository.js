...
(function(){
  ...
  function readHeader(){const q=s=>document.querySelector(s)?.value||'';return{nama_client:S(q('#qc')),perusahaan:S(q('#qp')),telepon_wa:S(q('#qw')),telepon:S(q('#qw')),whatsapp:S(q('#qw')),email:S(q('#qe')),nama_event:S(q('#qeve')),event_name:S(q('#qeve')),event:S(q('#qeve')),venue:S(q('#qvenue')),kota_venue:S(q('#qkota')),tanggal_mulai:q('#qs')||'',tanggal_selesai:q('#qe2')||'',status:'DRAFT',nomor_penawaran:'',diskon:I(window.__pmDiscountValue),diskon_persen:Math.max(0,Math.min(100,N(window.__pmDiscountPct))),diskon_nominal:I(window.__pmDiscountValue)};}
  ...
  async function ensureProjectForQuotation(quoteId,q){
    const d=DB(); if(!d||!quoteId)return;
    const existing=await d.from('project_penawaran').select('project_id').eq('penawaran_id',quoteId).limit(1);
    if(existing.error)throw existing.error;
    if((existing.data||[]).length)return Number(existing.data[0].project_id);
    const project={
      client_id:q.client_id||null,
      nama_project:S(q.nama_event)||('Project '+S(q.nomor_penawaran||quoteId)),
      nama_event:S(q.nama_event)||null,
      venue:S(q.venue)||null,
    ...
    const projectId=Number(ins.data?.id||0);
    if(!projectId)throw Error('Project otomatis tidak mendapatkan ID.');
    const code='PRJ-'+new Date().toISOString().slice(0,7).replace('-','')+'-'+String(projectId).padStart(4,'0');
    const up=await d.from('project_events').update({kode_project:code}).eq('id',projectId);
    if(up.error)throw up.error;
    const link=await d.from('project_penawaran').insert([{project_id:projectId,penawaran_id:quoteId}]);
