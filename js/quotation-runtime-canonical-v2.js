...
(function () {
  ...
  function itemSummary(item){
    const t=typeOf(item),label=S(item.item)||'Item belum dipilih',sub=itemSubtotal(item),schedule=item.mulai&&item.selesai?`${item.mulai} → ${item.selesai}`:'Jadwal belum lengkap';
    let metric='';
