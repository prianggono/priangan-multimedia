...
function itemType(i,m){const raw=S(i.tipe_perhitungan||i.tipe).toLowerCase();if(['level','rigging','luas','qty'].includes(raw))return raw;return masterType(m)}
...
async function load(from='',to=''){
 ...
 const paymentRows=pr.data||[];window.__PM_FINANCE_PAYMENT_ROWS=paymentRows;
 const paidBy=new Map();paymentRows.forEach(x=>{const k=S(x.penawaran_id);paidBy.set(k,(paidBy.get(k)||0)+N(x.nominal))});
 ...
 const rows=(qr.data||[]).filter(r=>{const dt=dateOf(r);return(!from||dt>=from)&&(!to||dt<=to)}).map(r=>{
  ...
  const base=N(r.subtotal)||details.reduce((a,x)=>a+N(x.item.subtotal),0);
  ...
  const modal=details.reduce((a,x)=>a+x.cost,0),laba=total-modal,margin=total>0?laba/total*100:0;
 ...
 render(rows,from,to,today,paymentRows);
}
function sum(rows,key){return rows.reduce((a,r)=>a+N(r[key]),0)}
function render(rows,from,to,today,paymentRows){
 const active=rows.filter(r=>!r.cancel),inv=active.filter(r=>r.inv),omzet=sum(active,'total'),modal=sum(active,'modal'),laba=sum(active,'laba'),disc=sum(active,'disc'),paid=sum(inv,'paid'),receivable=sum(inv,'balance'),invoiceUnpaid=inv.filter(r=>r.balance>0).length,overdue=inv.filter(r=>r.overdue).length,margin=omzet>0?laba/omzet*100:0,invoiceTotal=sum(inv,'invTotal'),lunas=inv.filter(r=>r.balance<=0).length,draft=active.filter(r=>r.draft).length,deal=active.filter(r=>r.deal).length,cancel=rows.filter(r=>r.cancel).length;
 ...
 <div class="grid g4" style="margin-top:16px"><div class="card stat"><small>Penawaran</small><strong>${active.length}</strong></div><div class="card stat"><small>DRAFT</small><strong>${draft}</strong></div><div class="card stat"><small>TERKIRIM / DEAL</small><strong>${deal}</strong></div><div class="card stat"><small>Rata-rata Nilai</small><strong>${M(active.length?omzet/active.length:0)}</strong></div></div>
 <div class="card" style="margin-top:16px"><div><b>Detail Keuangan per Penawaran / Invoice</b><div style="color:var(--muted);font-size:12px;margin-top:4px">Laba kotor = total penawaran setelah diskon − estimasi harga modal.</div></div><div class="scroll" style="margin-top:12px"><table class="table"><thead><tr><th>Tanggal</th><th>No Penawaran</th><th>Invoice</th><th>Client</th><th>Event</th><th>Total</th><th>Modal</th><th>Laba Kotor</th><th>Margin</th><th>Dibayar</th><th>Sisa</th><th>Tempo</th><th>Status</th></tr></thead><tbody>${active.map(x=>`<tr><td>${D(dateOf(x.r))}</td><td>${E(x.r.nomor_penawaran||x.r.nomor||'-')}</td><td>${E(invoiceNo(x.r)||'-')}</td><td>${E(x.r.nama_client||x.r.client_name||'-')}</td><td>${E(x.r.event_name||x.r.nama_event||'-')}</td><td>${M(x.total)}</td><td>${M(x.modal)}</td><td><b>${M(x.laba)}</b></td><td><b>${P(x.margin)}</b></td><td>${x.inv?M(x.paid):'-'}</td><td>${x.inv?M(x.balance):'-'}</td><td>${x.due?D(x.due):'-'}</td><td>${x.inv?(x.overdue?'JATUH TEMPO':x.balance<=0?'LUNAS':'BELUM LUNAS'):S(x.r.status||'DRAFT')}</td></tr>`).join('')||'<tr><td colspan="13" class="empty">Belum ada data.</td></tr>'}</tbody></table></div></div>
 <div class="grid g2" style="margin-top:16px"><div class="card"><b>Arus Kas / Pembayaran</b><div class="sum" style="margin-top:12px"><span>Kas masuk tercatat</span><b>${M(cashIn)}</b></div><p style="color:var(--muted);font-size:12px;margin:8px 0 0">Total pembayaran yang tercatat untuk penawaran dalam periode filter, termasuk DP dan pelunasan.</p></div><div class="card"><b>Kesehatan Piutang</b><div class="sum" style="margin-top:12px"><span>Belum lunas</span><b>${invoiceUnpaid} invoice</b></div><div class="sum" style="margin-top:8px"><span>Jatuh tempo</span><b>${overdue} invoice</b></div><div class="sum" style="margin-top:8px"><span>Total piutang</span><b>${M(receivable)}</b></div></div></div>
