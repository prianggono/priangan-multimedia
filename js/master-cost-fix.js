...
function packageEditor(row){
  const packageMode=!row || String(row?.satuan||'').trim().toLowerCase()==='paket' || !!String(row?.isi_paket||'').trim();
  const rows=parsePackageRows(row?.isi_paket||'');
  const initial=rows.length?rows:[{komponen:'',qty:'-'}];
  return `<div id="pmPackageEditor" class="pm-package-master-editor" ${packageMode?'':'hidden'}><div class="pm-package-master-head"><div><b>Isi Paket</b><div>Tambahkan komponen dan Qty yang termasuk dalam paket.</div></div><button type="button" class="btn secondary sm" id="pmAddPackageComponent">+ Tambah Komponen</button></div><div id="packageItems">${initial.map(packageRowHtml).join('')}</div><div class="pm-package-master-note">Data ini akan disimpan ke Master Harga dan ditampilkan pada tombol <b>Lihat Isi Paket</b>.</div></div>`;
}
function bindPackageEditor(){
  ...
  const refreshVisibility=()=>{if(!editor||!satuan)return;const isPackage=String(satuan.value||'').trim().toLowerCase()==='paket'||!!list?.querySelector('.pm-package-component')&&[...list.querySelectorAll('.pm-package-component')].some(x=>String(x.value||'').trim());editor.hidden=!isPackage;};
  const add=()=>{
    if(!list)return;
  ...
  document.getElementById('pmAddPackageComponent')?.addEventListener('click',e=>{e.preventDefault();add();});
  list?.addEventListener('click',e=>{const b=e.target.closest('.pm-package-remove');if(!b)return;e.preventDefault();const rows=[...list.querySelectorAll('.pm-package-edit-row')];if(rows.length===1){rows[0].querySelector('.pm-package-component').value='';rows[0].querySelector('.pm-package-qty').value='-';return}b.closest('.pm-package-edit-row')?.remove();});
  satuan?.addEventListener('input',refreshVisibility);
  satuan?.addEventListener('change',refreshVisibility);
  refreshVisibility();
}
function installPackageStyles(){
  ...
  const st=document.createElement('style');st.id='pmPackageMasterEditorStyles';st.textContent=`
    ...
    .pm-package-master-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:12px}
